...
def loss_fn_maxutil(y_pred_batch, y_true_batch, env):
    ...
    for i in range(batch_size):
        ...
        if props.so_mode == 'test' and batch_size == 1:
            ret_paths_split = paths_split
        ...
        if props.so_mode == 'test' and batch_size == 1:
            max_cong_vals.append(max_cong)
            opt_vals.append(opt)

    ret = sum(losses) / len(losses)
    ret_val = sum(loss_vals) / len(loss_vals)
    if props.so_mode == 'test' and batch_size == 1:
        ret_max_cong = sum(max_cong_vals) / len(max_cong_vals)
        ret_opt = sum(opt_vals) / len(opt_vals)

    if props.so_mode == 'test' and batch_size == 1:
        return ret, ret_val, ret_max_cong, ret_opt, ret_paths_split
    return ret, ret_val
...
commodities_to_paths = torch.sparse_coo_tensor(np.vstack((ctp_coo.row, ctp_coo.col)), torch.DoubleTensor(ctp_coo.data),
                                               torch.Size(ctp_coo.shape))
pte_coo = env._optimizer._paths_to_edges.tocoo()
paths_to_edges = torch.sparse_coo_tensor(np.vstack((pte_coo.row, pte_coo.col)), torch.DoubleTensor(pte_coo.data),
                                         torch.Size(pte_coo.shape))
...
if props.so_mode == SOMode.TRAIN:  # train
    ...
    # load the model
    # model_name = 'model_dote_GEANT.pkl'
    # model_name = 'model_dote_GEANT_SHRINK_4.pkl'
    model_name = 'model_dote_GEANT_SHRINK_5.pkl'
    # model_name = 'model_dote_Abilene.pkl'
    # log_name = model_name[0:-4] + '_burst_log.txt'
    # log_name = model_name[0:-4] + '_gravity_4_6_log.txt'
    log_name = model_name[0:-4] + '_log.txt'
    ...
    with torch.no_grad():
        with tqdm(test_dl) as tests:
            test_losses = []
            cnt = 13
            print(len(tests))  # 2016-12
            # input: previous 12 tm
            # target: current tm & its optimal solution
            if not os.path.exists('../logs'): os.mkdir('../logs')
            with open('../logs/' + log_name, 'w') as f:
                # with open('../6_hist_log.txt', 'w') as f:
                header = 'cnt max_cong opt loss_val [paths_split]\n'
                f.write(header)
                ...
                for (inputs, targets) in tests:
                    ...
                    pred = model(inputs)
                    # print(type(inputs))
                    # pred = model(first_input)
                    # print(targets)
                    test_loss, test_loss_val, max_cong, opt, paths_split = loss_fn(pred, targets, env)
                    test_losses.append(test_loss_val)
                    print(f'{cnt} loss_val= {test_loss_val}')

                    paths_split = paths_split.numpy().flatten()
                    paths_split = paths_split.tolist()

                    line = f'{cnt}\t{max_cong}\t{opt}\t{test_loss_val}\t{paths_split}\n'
                    f.write(line)
                    # f.write()
                    cnt += 1
